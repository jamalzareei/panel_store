(function($) {
    ...
    $(".chat-application .chat-user-list ul li").on('click', function() {
        ...
        $.ajax({
            ...
            data: { ajax: 'true' },
            beforeSend: function() {
                $('#load-chat').html('loading ... ')
            },
            success: function(response) {
                $('#load-chat').html(response.view_chat)
                $('#load-header').html(response.view_header)
                $('#load-form').html(response.view_form)

                $('#load-chat').animate({ scrollTop: chatContainer[0].scrollHeight }, 400)
                $('.user-chats').animate({ scrollTop: chatContainer[0].scrollHeight }, 400)
            },
...
// Add message to chat
function enter_chat(url) {
    var message = $("#txt-message").val();
    var sender_id = $("#txt-sender_id").val();
    if (message != "") {
        ...
        $.ajax({
            ...
            method: 'post',
            method: 'post',
            data: { ajax: 'true', message: message, sender_id: sender_id },
            beforeSend: function() {
                $('#btn-send').html('<i class="fas fa-circle-notch fa-spin"></i>')
            },
            success: function(response) {
                $(".chat:last-child .chat-body").append(html);
                $("#txt-message").val("");
                $(".user-chats").scrollTop($(".user-chats > .chats").height());
                $('#btn-send').html('<i class="fa fa-paper-plane-o d-lg-none"></i> <span class="d-none d-lg-block">ارسال</span>')
            },
            error: function(request, status, error) {}
        })
