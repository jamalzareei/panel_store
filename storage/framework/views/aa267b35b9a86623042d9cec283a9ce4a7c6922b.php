                ...
                <ul class="nav navbar-nav float-right">
                    <?php if($user->roles->where('slug', 'SELLER')->first() && !$user->roles->where('slug', 'ADMIN')->first()): ?>
                        ...
                        <li class="nav-item d-none d-lg-block">
                            ...
                            <?php elseif($user && $user->seller && ( !$user->seller->image || !isset($user->seller->image[0]) || !isset($user->seller->image[0]->path ))): ?>
                                <a class="btn btn-primary text-white my-1" href="<?php echo e(route('seller.data.get')); ?>" >
                                    آپلود لوگو
                                </a>
                            <?php elseif($user && $user->seller && (!$user->seller->sell_type_id || !$user->seller->pay_type_id)): ?>
                                <a class="btn btn-primary text-white my-1" href="<?php echo e(route('seller.setting.get')); ?>" >
                                    تنظیمات فروشگاه
                                </a>
                            <?php elseif($user && $user->seller && !$user->seller->actived_at): ?>
                    ...
                    <div class="user-nav d-sm-flex d-none"><span class="user-name text-bold-600"><?php echo e(auth()->user()->full_name ?? ''); ?></span><span class="user-status">آنلاین</span></div><span><img class="round" src="<?php echo e(config('shixeh.cdn_domain')); ?><?php echo e($user->image[0]->path ?? '/assets/images/logo.png'); ?>" alt="avatar" height="40" width="40"></span>
                        </a>
